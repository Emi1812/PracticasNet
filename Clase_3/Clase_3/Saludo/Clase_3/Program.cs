
//Borra la pantalla
Console.Clear();

//Pedimos al usuario que introduzca su nombre
Console.WriteLine("Hola, Ingrese su nombre: ");

//Leemos el nombre del usuario
string nombre;                      //creamos la variable           
nombre=Console.ReadLine();          // Asignamos a la variable lo que escriba el usuario

// Pedimos al usuari que introduzca su apellido
Console.WriteLine("Ahora por favor ingrese su apellido: ");

string apellido;
apellido = Console.ReadLine();

//Mostramos el nombre
Console.Write("Hola " + nombre + " " + apellido); //No pasa a la siguiente linea
Console.WriteLine(", este es mi segundo programa");


Console.WriteLine("Pulse una tecla para finalizar");
Console.ReadKey();
